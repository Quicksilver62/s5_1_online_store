...
public class CartPageTest {
    ...
    void cartPage_ShouldReturnCartViewWithItems() {
        ...
        Mono<CartModel> mockModel = Mono.just(new CartModel(mockItems, 300.0, true));
                ...
                when(cartFacade.getCartModel()).thenReturn(mockModel);
    ...
    @Test
    void cartPage_ShouldReturn_NotAuthenticated() {
        webTestClient.get()
                .uri("/cart/items")
                .exchange()
    ...
    void handleItemAction_WithPlusAction_ShouldUpdateCart() {
        ...
        when(cartFacade.handleItemActionAndGetModel(anyString(), anyInt())).thenReturn(mockModel);
    ...
    void buy_ShouldClearCart() {
        when(cartService.buy()).thenReturn(Mono.empty());
