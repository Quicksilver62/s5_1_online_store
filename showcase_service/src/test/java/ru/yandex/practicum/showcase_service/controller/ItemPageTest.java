...
public class ItemPageTest {
    ...
    void itemPage_ShouldReturnItemView() {
        ...
        when(itemService.getItem(eq(1))).thenReturn(Mono.just(mockItem));
    ...
    @Test
    void itemPage_ShouldReturnItemView_NotAuthenticated() {
        ItemDto mockItem = new ItemDto(1, "Test Item", "Description", "/img.jpg", 100.0, 0);
        when(itemService.getItem(eq(1))).thenReturn(Mono.just(mockItem));
    ...
    void handleItemAction_WithPlusAction_ShouldUpdateItem() {
        ...
        when(itemService.getItem(eq(1))).thenReturn(Mono.just(updatedItem));
        when(cartService.handleItemAction(eq("plus"), eq(1))).thenReturn(Mono.empty());
